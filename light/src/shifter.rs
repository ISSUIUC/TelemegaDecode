...
impl Shifter {
    pub fn new(freq: f64, hz: f64) -> Shifter {
        ...
        let mut signed_sin_len = (hz / freq * best_repeat_ct as f64).round();
        if signed_sin_len == 0.0 {
            signed_sin_len = 1.0;
        }
        ...
        let buffer = (0..sin_len).map(|i| {
            let angle = 2.0 * std::f64::consts::PI / signed_sin_len * (i * best_repeat_ct) as f64;
            Complex::from_polar(1.0, angle as f32)
        }).collect();
